...
public class DoWhileSwimming {
    public static void main(String[] args) throws Exception{
        ...
        double minimymTemperature = 79.0;  // degrees Fahrengeit
        ...
        System.out.println("\nOkay, so the current water temperature is " + currentTemperature);
        System.out.println(swimmer1 + " approaches the lake...");

        swimTime = 0;
        while (currentTemperature >= minimymTemperature){
            System.out.print("\t" + swimmer1 + " swims for a bit.");
            swimTime++;
            System.out.println(" Swim time: " + swimTime + " min.");
            Thread.sleep(600);    // pauses for 600 milliseconds
            currentTemperature -= 0.5; // subtract 1/2 a degree from the water temperature
            System.out.println("The current water temperature is now " + currentTemperature + "F.");
        }

        System.out.println(swimmer1 + " stops swimming. Total swim time: " + swimTime + " min.");

        currentTemperature = savedTemperature;  // restore original water temperature
